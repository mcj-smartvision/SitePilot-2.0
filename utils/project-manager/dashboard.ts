...
export function buildProjectHealth(
  ...
  const delayDays = summary.delayedTasks > 5 ? 3 : summary.delayedTasks > 0 ? 1 : 0
  const pending = approvals.filter((a) => a.status === 'pending_pm').length

  let riskLevel: ProjectHealthStatus['riskLevel'] = 'low'
  if (summary.delayedTasks > 5 || pending > 5 || lowStock > 3) riskLevel = 'high'
  else if (summary.delayedTasks > 0 || pending > 0 || lowStock > 0) riskLevel = 'medium'

  return {
    plannedProgress: Math.min(100, summary.overallPercentComplete + 5),
    actualProgress: summary.overallPercentComplete,
...
export async function loadProjectManagerDashboard(
  ...
  const [aiRows, inventory] = await Promise.all([
  ...
  const health = buildProjectHealth(summary, inventory, approvals, alerts)
  const departments = buildDepartmentSummaries(approvals, alerts)
  const activity = buildActivityFeed(approvals, alerts, reports)

  return { health, approvals, departments, activity, summary, reports }
}
